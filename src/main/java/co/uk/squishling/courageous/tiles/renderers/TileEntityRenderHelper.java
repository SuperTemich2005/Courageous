...
public class TileEntityRenderHelper {
    public static void DrawFluidPlane(IVertexBuilder builder, MatrixStack matrixStack, Fluid fluid, World world, BlockPos pos, Float xMin, Float zMin, Float xMax, Float zMax, Float height, int overlay) {
        ...
        int light = Math.max(fluid.getAttributes().getLuminosity(), world.getLightFor(LightType.BLOCK, pos)) * 16; //Luminosity is in light levels. By multiplying by 16 we get a 0-256 level. Also, figure out whether the fluid itself gives off more light than the outside light, and if so, use that.
